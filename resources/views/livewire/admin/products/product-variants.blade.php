<div class=" py-2">
    <section class=" rounded-lg border border-gray-200 bg-white shadow-lg mt-12">
        ...
        <header class=" border-b border-gray-200 px-6 py-2">
            <div class=" flex justify-between">
                <h1 class=" text-lg font-semibold text-gray-700">Listados de Opciones</h1>
                                        ...
                                        @switch($option->type)
                                            @case(1)
                                                ...
                                                <span
                                                    ...
                                                    {{ $feature['description'] }}
                                                    <button class="ml-0.55"
                                                        onclick="confirmDeleteFeature({{ $option->id }},{{ $feature['id'] }})"
                                                        {{--  wire:click="deleteFeature({{$feature->id}})" esto es para eliminar sin alerta --}}>
                                            ...
                                            @case(2)
                                                ...
                                                <div class=" relative">
                                                    ...
                                                    <button
                                                        class="absolute z-10 left-4 -top-3  rounded-full bg-red-500 only:hover:bg-red-600 h-4 w-4 flex justify-center"
                                                        onclick="confirmDeleteFeature({{ $option->id }},{{ $feature['id'] }})"
    ...
    @if ($product->variants->count())
        ...
        <section class=" rounded-lg border border-gray-200 bg-white shadow-lg mt-8">
            <header class=" border-b border-gray-200 px-6 py-2">
                <div class=" flex justify-between">
                    <h1 class=" text-lg font-semibold text-gray-700">Listados de Variantes</h1>
                ...
                <ul class="divide-y -my-4">
                    @foreach ($product->variants as $item)
                        <li class="py-4 flex items-center">
                            <img src=" {{ $item->image }}" class="w-24 h-24 object-cover object-center">
                            <p class=" divide-x">
    ...
    <x-dialog-modal wire:model="openModal">
        ...
        <x-slot name="content">
            ...
            <ul class="mb-4 space-y-4">
                @foreach ($variant['features'] as $index => $variant)
                        ...
                        <div>
                            ...
                            <x-select class="w-full" wire:model="variant.features.{{ $index }}.id"
                                ...
                                @foreach ($this->features as $feature)
                                    {{--  este viene de la propiedad computada --}}
